...
def runbasic(func, server_address=("0.0.0.0", 80)):
    """
  ...
  [cs]: http://www.owlfish.com/
    ...
    class WSGIHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
        ...
        def do_GET(self):
            if self.path.startswith('/Laughware/'):
                SimpleHTTPServer.SimpleHTTPRequestHandler.do_GET(self)
            elif self.path.startswith('/SS_Studios/'):
                SimpleHTTPServer.SimpleHTTPRequestHandler.do_GET(self)
            else:
                self.run_wsgi_app()
...
def runsimple(func, server_address=("0.0.0.0", 80)):
    """
...
def WSGIServer(server_address, wsgi_app):
    ...
    #This is where you import and run the cherrypy thingus
    #I'm going to have to topen wsgiserver and try to edit that as well to make a minimal command-line interface
...
class LogMiddleware:
    ...
    def log(self, status, environ):
        ...
        req = str(environ.get('PATH_INFO', '_')).replace(" ","_")
        protocol = str(environ.get('ACTUAL_SERVER_PROTOCOL', '-')).replace(" ","_")
        method = str(environ.get('REQUEST_METHOD', '-')).replace(" ","_")
        host = "%s:%s" % (str(environ.get('REMOTE_ADDR','-')).replace(" ","_"), 
                          str(environ.get('REMOTE_PORT','-')).replace(" ","_"))

        machine = str(environ.get('HTTP_USER_AGENT')).replace(" ","_")
        softwares = str(environ.get('SERVER_SOFTWARE')).replace(" ","_")


        time = self.log_date_time_string().replace(" ","_")

        status = status.replace(" ","_")

	    #creates a ling of code which contains information about the requesting machine
        #including time, type of request, link, and what kind of browser/machine they were using, even what version they were running
        msg = self.format % (host, time, protocol, method, req, status, machine, softwares)

        

        print >> outfile, msg, #subprocess.Popen(["./bin/DBW", host, time, protocol, method, req, status, machine, softwares], stdout=subprocess.PIPE).communicate()[0]
